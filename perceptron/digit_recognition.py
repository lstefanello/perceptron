...
def main():
    ...
    network = pc.network([784, 800, 10, 10], ("relu", "relu", "softmax"), training_data*(1/255), labels)
    learn = (network, 0.0001, 0.1)
    network.train(lr_func="warm restarts", lr_params=learn, batch_size=20, epochs=30)
    #network.train(lr_func="const", lr_params=0.1, batch_size=20, epochs=30, cost_func="quadratic")
